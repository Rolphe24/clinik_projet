...
def ajout_patients(request):
    ...
    if request.method == 'POST':
        ...
        if est_hospitalise and not a_consulte:
            return HttpResponse("Un patient ne peut être hospitalisé que s'il a été consulté.")
...
def vendre_produit(request):
    if request.method == 'POST':
        ...
        return redirect('remerciment')  # Rediriger vers la liste des produits après la vente
...
def enregistrer_utilisateur(request):
    if request.method == 'POST':
        ...
        return redirect('connexion_utilisateur')  # Rediriger vers une vue qui affiche la liste des utilisateurs
...
def connexion_utilisateur(request):
    ...
    if request.method == 'POST':
        ...
        if utilisateur is not None and check_password(password, utilisateur.password):
            ...
            return render(request, 'connexion_utilisateur.html', {'erreur': erreur, 'noms_utilisateurs': noms_utilisateurs, 'nom_pre_rempli': nom})

    return render(request, 'connexion_utilisateur.html', {'noms_utilisateurs': noms_utilisateurs})
...
def rapport_mensuel(request):
    rapport, mois_formatte = RapportMensuel.generer_rapport()
    context = {
        'rapport': rapport,
        'mois': mois_formatte,
    }
